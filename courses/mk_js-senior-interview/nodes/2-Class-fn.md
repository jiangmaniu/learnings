...
语法糖与原始写法本质是一样的（效果或结果），但语法糖会比原始的写法会更加简单，易懂，清晰。可以理解为语法糖的代码会被解析成原始代码。

JS中没有真真意义上的`Class`，只是为了让类的写法与`java` `C#`等语言的类看起来一样，在语法上更加贴合面向对象的写法，实现继承更加易读、易理解，但本质上还是通过 `prototype` 原型链的形式运行的。
